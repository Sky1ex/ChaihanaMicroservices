...
namespace WebApplication1.Services
{
    public class AccountService : IAccountService
    {
        ...
        public AccountService(ILogger<CartService> logger, IUnitOfWorkCustomers unitOfWorkForCustomers, IMapper mapper, IUnitOfWorkCafe unitOfWorkForCafe)
        {
            _logger = logger;
            _unitOfWorkForCustomers = unitOfWorkForCustomers;
            _mapper = mapper;
            _unitOfWorkForCafe = unitOfWorkForCafe;
        }

        public async Task<List<AddressDto>> GetAddresses(Guid userId)
        {
            var user = await _unitOfWorkForCustomers.Users.GetByIdWithAddresses(userId);
        ...
        public async Task<List<OrderDto>> GetOrders(Guid userId)
        {
            ...
            var user = await _unitOfWorkForCustomers.Users.GetByIdFull(userId);

            List<OrderDto> orders = new List<OrderDto>();

            foreach(var order in user.Orders)
            {
				var productIds = order.OrderElement.Select(ce => ce.ProductId).Distinct().ToList();
				var products = _unitOfWorkForCafe.Products.GetProducts(productIds);
				var productDict = products.ToDictionary(p => p.ProductId);
                var orderDto = _mapper.Map<OrderDto>(order);

				foreach (var element in orderDto.Products)
				{
        ...
        public async Task<string> CheckCode(string code, Guid userId)
        {
            ...
            else
            {
                ...
                var checkUser = await _unitOfWorkForCustomers.Users.GetByPhone(codeDto.number);

                if (checkUser != null)
                {
                    var user = await _unitOfWorkForCustomers.Users.GetByIdAsync(userId);

                    _unitOfWorkForCustomers.Users.Delete(user);
                    await _unitOfWorkForCustomers.SaveChangesAsync();
        ...
        public async Task<bool> AddAddress(string City, string Street, string House, int Apartment, Guid userId)
        {
            ...
            await _unitOfWorkForCustomers.Addresses.AddAsync(address);
            await _unitOfWorkForCustomers.SaveChangesAsync();
            return true;
        }
